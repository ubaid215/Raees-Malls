...
const OrderManagement = () => {
  ...
  const { admin, isAdminAuthenticated, logoutAdmin, refreshAdminToken } =
    useAdminAuth();
...
const getItemImage = (item) => {
  ...
  const fetchOrders = useCallback(
    async (
      ...
      try {
        console.log(
          "OrderManagement: Fetching orders for page:",
          page,
          "limit:",
          limit,
          "status:",
    ...
    const setupSocket = () => {
      try {
        if (!socketService.getConnectionState()) {
          ...
          console.log(
            "OrderManagement: Socket connection established for admin:",
            admin?._id
          );
        }
        ...
        socketService.on("orderCreated", (newOrder) => {
          console.log(
            "OrderManagement: New order received via socket:",
            newOrder
          );
          if (!newOrder || !newOrder.orderId) {
            console.error(
              "OrderManagement: Invalid order data received:",
              newOrder
            );
            return;
          }
        ...
        socketService.on("orderStatusUpdated", (updatedOrder) => {
          console.log(
            "OrderManagement: Order status updated via socket:",
            updatedOrder
          );
  ...
  useEffect(() => {
    ...
    console.log(
      "OrderManagement: Fetching orders on mount for admin:",
      admin._id
    );
    ...
    const pingInterval = setInterval(() => {
      try {
        if (socketService.getConnectionState()) {
          ...
          console.log(
            "OrderManagement: Socket not connected, attempting to reconnect"
          );
          socketService.connect(admin?._id, admin?.role);
        }
      } catch (err) {
  ...
  const handleSearch = async (e) => {
    ...
    if (searchTerm) {
      if (searchTerm.includes("@")) {
        ...
        toast.info(
          "Please enter a valid order ID (ORD- followed by 8 characters) or customer email"
        );
      }
    } else {
      fetchOrders(1, pagination?.limit || 10, filterStatus);
  ...
  if (admin.role !== "admin" && admin.role !== "administrator") {
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4">
        <div className="bg-white shadow-lg rounded-lg p-6 max-w-sm w-full text-center">
          <p className="text-red-600 text-sm mb-4">
            Unauthorized access. Your role ({admin.role}) is not permitted to
            view this page.
          </p>
          <Button
            onClick={() => logoutAdmin()}
      ...
      <div className="max-w-5xl mx-auto">
            ...
            <p className="text-sm text-gray-600 mt-1">
              {pendingOrdersCount > 0 ? (
                <span className="flex items-center">
                  ...
                  pending orders require your attention
                </span>
              ) : (
        ...
        >
          ...
          {socketService.getConnectionState()
            ? "Connected to server"
            : "Disconnected - Click refresh to reconnect"}
        </div>
          ...
          <>
            {(orders || []).map((order) => {
              if (!order || !order.orderId) {
                console.warn(
                  "OrderManagement: Invalid order data skipped:",
                  order
                );
                return null;
              }
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-4">
              ...
              <p className="text-sm text-gray-600">
                Page {pagination.page} of {pagination.totalPages} (Total Orders:{" "}
                {pagination.total})
              </p>
              <Button
                onClick={() => handlePageChange(pagination.page + 1)}
